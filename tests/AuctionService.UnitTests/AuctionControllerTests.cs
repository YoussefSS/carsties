...
public class AuctionControllerTests
{
    ...
    public async Task GetAuctionById_WithValidGuid_ReturnsAuction()
    {
        ...
        // remember that we're not testing the database here, so the Guid does not matter
    ...
    public async Task UpdateAuction_WithUpdateAuctionDto_ReturnsOkResponse()
    {
        ...
        auction.Seller = "test"; // same as the Name in Helpers.GetClaimsPrincipal()
    ...
    public async Task UpdateAuction_WithInvalidUser_Returns403Forbid()
    {
        // arrange
        var auction = _fixture.Build<Auction>().Without(x => x.Item).Create();
        auction.Seller = "not-test"; // different from the one in Helpers.GetClaimsPrincipal()
        var updateDto = _fixture.Create<UpdateAuctionDto>();
        _auctionRepo.Setup(repo => repo.GetAuctionEntityById(It.IsAny<Guid>())).ReturnsAsync(auction);
